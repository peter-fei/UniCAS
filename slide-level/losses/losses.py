...
class Ralloss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if x.size(-1) == 1:
            ...
            # xs_pos = logpt.data.exp()
            xs_pos = torch.exp(logpt)
            # print((xs_pos==a).sum()/a.numel())
            xs_neg = 1 - xs_pos
        ...
        # print(los_pos.sum().item(),los_neg.sum().item(),(1-y).sum().item(),(torch.log(xs_neg.clamp(min=self.eps)) + self.epsilon_neg * (xs_neg.clamp(min=self.eps))).max().item(),(self.lamb - xs_pos).max().item(),(xs_pos ** 2).max().item(),(self.lamb - xs_neg).max().item())
        # print(y.size(),los_pos.size(),x.size())
        # Asymmetric Focusing
